...
//template <typename T, uint8_t N, T default_value> c

class FloatingFastRunningMedian {

public:
	FloatingFastRunningMedian(uint8_t N, double default_value) {
		_buffer_ptr = N;
		_window_size = N;
		_median_ptr = N/2;
	...
	void addValue(double new_value) {
		...
		double old_value = _inbuffer[_buffer_ptr]; // retrieve the old value to be replaced
